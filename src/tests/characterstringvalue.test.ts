...
describe('CharacterStringValue (writable, priority array)', () => {
  ...
  it('should have all-null Priority_Array initially', async () => {
    const value = await bsReadProperty(1, ObjectType.CHARACTERSTRING_VALUE, 1, PropertyIdentifier.PRIORITY_ARRAY);
  ...
  it('should have Relinquish_Default matching the initial presentValue', async () => {
    const value = await bsReadProperty(1, ObjectType.CHARACTERSTRING_VALUE, 1, PropertyIdentifier.RELINQUISH_DEFAULT);
    deepStrictEqual(value.trim(), '"default"');
  });

  it('should read Current_Command_Priority as 16 initially', async () => {
    const value = await bsReadProperty(1, ObjectType.CHARACTERSTRING_VALUE, 1, PropertyIdentifier.CURRENT_COMMAND_PRIORITY);
    deepStrictEqual(parseInt(value.trim()), 16);
  });

  it('should update Present_Value when writing at priority 8', async () => {
    await bsWriteProperty(1, ObjectType.CHARACTERSTRING_VALUE, 1, PropertyIdentifier.PRESENT_VALUE, 8, ApplicationTag.CHARACTER_STRING, 'updated');
    const value = await bsReadProperty(1, ObjectType.CHARACTERSTRING_VALUE, 1, PropertyIdentifier.PRESENT_VALUE);
    deepStrictEqual(value.trim(), '"updated"');
  });
  ...
  it('should revert Present_Value to Relinquish_Default after releasing priority with NULL', async () => {
    await bsWriteProperty(1, ObjectType.CHARACTERSTRING_VALUE, 1, PropertyIdentifier.PRESENT_VALUE, 8, ApplicationTag.CHARACTER_STRING, 'updated');
    await bsWriteProperty(1, ObjectType.CHARACTERSTRING_VALUE, 1, PropertyIdentifier.PRESENT_VALUE, 8, ApplicationTag.NULL, 0);
    const value = await bsReadProperty(1, ObjectType.CHARACTERSTRING_VALUE, 1, PropertyIdentifier.PRESENT_VALUE);
    deepStrictEqual(value.trim(), '"default"');
  });
